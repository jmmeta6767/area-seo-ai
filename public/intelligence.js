(() => {
  ...
  function draw(){
    ...
    const technicalIssues=selected.siteIssues||[],robots=selected.robots||{};
    for(const [label,value,note] of [['คะแนนเฉลี่ย',selected.average===null?'—':selected.average+'/100',delta(selected.delta)],['ตรวจแล้ว',selected.pages.length+' หน้า','เทียบได้ '+selected.comparedPages+' หน้าเดิม'],['จุดปรับปรุง',selected.issueCount,'อ่านไม่ได้ '+selected.failedPages+' หน้า'],['Technical SEO',technicalIssues.length+' จุด',robots.status?'robots.txt HTTP '+robots.status:'ยังไม่มีผล robots.txt'],['แก้แล้วจากรอบก่อน',selected.resolvedCount,'ตรวจพบว่าปัญหาเดิมหายไป']]){const card=make('article');card.append(make('small',label),make('b',value),make('span',note));stats.append(card);}
    el('scanCoverage').textContent='สแกน '+date(selected.scannedAt)+' · พบ '+(selected.discoveryLimited?'อย่างน้อย ':'')+selected.discovered+' URL · '+(selected.sitemapsRead?'อ่าน sitemap '+selected.sitemapsRead+' ไฟล์ · ':'')+(robots.readable?(robots.blocksAll?'robots.txt บล็อกทั้งเว็บ · ':'robots.txt อ่านได้ · '):robots.status?'robots.txt HTTP '+robots.status+' · ':'')+(selected.partial?'ผลบางส่วน':'ครบชุด URL ที่พบ')+' · '+selected.site+(selected.warnings.length?' · '+selected.warnings.join(' · '):'');
    const tasks=[...technicalIssues.map(i=>({...i,url:selected.site,newIssue:false,siteIssue:true})),...selected.pages.flatMap(p=>p.issues.map(i=>({...i,url:p.url,newIssue:p.newIssues.includes(i.key)})))].sort((a,b)=>({high:0,medium:1,low:2}[a.priority]-{high:0,medium:1,low:2}[b.priority]));

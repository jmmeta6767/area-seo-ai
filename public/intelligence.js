(() => {
  ...
  function draw(){
    ...
    el('scanCoverage').textContent='สแกน '+date(selected.scannedAt)+' · พบ '+selected.discovered+' URL · '+(selected.partial?'ผลบางส่วน':'ครบชุด URL ที่พบ')+' · '+selected.site+(selected.warnings.length?' · '+selected.warnings.join(' · '):'');

...
function createPersistentStore({databaseUrl=process.env.DATABASE_URL}={}){
  ...
  async function health(){const t=Date.now();await pool.query('SELECT 1');return{ok:true,kind:'postgres',latencyMs:Date.now()-t}}
  return {configured:true,kind:'postgres',init,get,set,audit,events,health,close:()=>pool.end()};
}

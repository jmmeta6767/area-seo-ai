...
function createPersistentStore({databaseUrl=process.env.DATABASE_URL}={}){
  ...
  async function stateStatus(){
    const expected=['approval_queue','site_history','quality_history'];
    const r=await pool.query('SELECT key,updated_at FROM area_seo_state WHERE key = ANY($1::text[]) ORDER BY key',[expected]);
    const present=r.rows.map(x=>x.key),missing=expected.filter(k=>!present.includes(k));
    return{expected,present,missing,complete:missing.length===0,updatedAt:Object.fromEntries(r.rows.map(x=>[x.key,x.updated_at]))};
  }

...
function createApprovalStore({dataDir}){
  ...
  function cleanAudit(event){
    return {at:event?.at&&Number.isFinite(Date.parse(event.at))?new Date(event.at).toISOString():new Date().toISOString(),event:String(event?.event||''),draftId:event?.draftId||null,from:event?.from||null,to:event?.to||null,detail:event?.detail??null};
  }
  function audit(event){
    const clean=cleanAudit(event);
    fs.appendFileSync(auditFile,JSON.stringify(clean)+'\n',{mode:0o600});return clean;
  }
  function replaceAudit(events){
    const clean=(Array.isArray(events)?events:[]).map(cleanAudit).sort((a,b)=>a.at.localeCompare(b.at));
    fs.mkdirSync(dataDir,{recursive:true});const tmp=auditFile+'.'+randomUUID()+'.tmp';
    try{fs.writeFileSync(tmp,clean.map(x=>JSON.stringify(x)).join('\n')+(clean.length?'\n':''),{mode:0o600});fs.renameSync(tmp,auditFile)}
    finally{if(fs.existsSync(tmp))fs.unlinkSync(tmp)}
    return clean;
  }
  function allEvents(){if(!fs.existsSync(auditFile))return[];return fs.readFileSync(auditFile,'utf8').split('\n').filter(Boolean).map(line=>JSON.parse(line))}
  ...
  return {read,write:atomicWrite,audit,replaceAudit,allEvents,transition,events,queueFile,auditFile};
}

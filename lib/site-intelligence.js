...
function analyzePage(url, response) {
  ...
  let canonicalOK=false;try{const u=new URL(canonical);canonicalOK=!!canonical&&u.protocol==='https:'&&!u.username&&!u.password;}catch{}
...
function createIntelligence({dataDir,base,fetcher=fetchPage,cooldownMs=60000}) {
  base=allowedURL(base,base);const file=path.join(dataDir,'site-history.json');let running=false,lastStart=0;
  const read=()=>{if(!fs.existsSync(file))return {version:1,snapshots:[]};const data=JSON.parse(fs.readFileSync(file,'utf8'));if(data.version!==1||!Array.isArray(data.snapshots))throw Error('Cannot read site history; preserve file and restore a valid backup');return data;};
  const save=data=>{fs.mkdirSync(dataDir,{recursive:true});const temp=file+'.'+randomUUID()+'.tmp';try{fs.writeFileSync(temp,JSON.stringify(data),{mode:0o600});fs.renameSync(temp,file);}finally{if(fs.existsSync(temp))fs.unlinkSync(temp);}};
  ...
  async function scan() {
    ...
    try {
      const old=read(),warnings=[],urls=new Set([base]);let discovered=0;
      try {
        const map=await fetcher(new URL('/sitemap.xml',base).href,base);
        if(map.status!==200)throw Error('Sitemap HTTP '+map.status);
        if(!/<(?:urlset|sitemapindex)\b/i.test(map.html))warnings.push('รูปแบบ sitemap ไม่รองรับ: ตรวจได้เฉพาะหน้าแรก');
        if(/<sitemapindex\b/i.test(map.html))warnings.push('พบ sitemap index: รุ่นนี้สแกนหน้าแรกเท่านั้น ยังไม่ไล่ sitemap ย่อย');
        else for(const m of map.html.matchAll(/<loc\b[^>]*>\s*([\s\S]*?)\s*<\/loc>/gi)) {
          try{const u=allowedURL(decode(m[1].trim()),base);urls.add(u);}catch{warnings.push('ข้าม URL ที่อยู่นอกเว็บไซต์หรือรูปแบบไม่ถูกต้อง');}
        }
      }catch(e){warnings.push('อ่าน sitemap ไม่สำเร็จ: '+e.message);}
      ...
      const snapshot={id:randomUUID(),site:base,scoringVersion:VERSION,scannedAt:new Date().toISOString(),discovered,limit:LIMIT,partial:discovered>LIMIT||warnings.length>0||pages.some(p=>p.score===null),warnings:[...new Set(warnings)],average:scored.length?Math.round(scored.reduce((n,p)=>n+p.score,0)/scored.length):null,delta:compared.length?Math.round(compared.reduce((n,p)=>n+p.delta,0)/compared.length):null,comparedPages:compared.length,failedPages:pages.length-scored.length,issueCount:pages.reduce((n,p)=>n+p.issues.length,0),resolvedCount:pages.reduce((n,p)=>n+p.resolvedIssues.length,0),pages};
      old.snapshots.unshift(snapshot);old.snapshots=old.snapshots.slice(0,RETAIN);save(old);return snapshot;
    } finally {running=false;}
  }
  return {scan,history:summaries,latest:()=>read().snapshots.find(s=>s.site===base)||null,detail:id=>read().snapshots.find(s=>s.id===id&&s.site===base),status:()=>({running,retention:RETAIN,pageLimit:LIMIT,scoringVersion:VERSION,storageConfigured:!!process.env.DATA_DIR,storageNote:process.env.DATA_DIR?'ใช้ DATA_DIR ที่ตั้งค่าไว้ ต้องอยู่บนพื้นที่ถาวรเพื่อเก็บข้อมูลข้าม deploy':'เก็บไฟล์ในเครื่องเซิร์ฟเวอร์: Render แบบไม่มี persistent disk อาจสูญเสียประวัติเมื่อ deploy ใหม่'})};
}
module.exports={createIntelligence,analyzePage,allowedURL,publicIP,fetchPage};

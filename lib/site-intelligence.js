...
const scoredPage = p => Number.isFinite(p.score) && p.status>=200 && p.status<300;
const LIMIT = 100;
...
function analyzePage(url, response) {
  ...
  if(!(response.status>=200&&response.status<300)) {
    ...
    return {url,finalUrl:response.url||url,title:'',score:null,status,error:'HTTP '+status,checks:[],
      issues:[{key:'http',priority:'high',label:'หน้าเว็บตอบกลับไม่สำเร็จ',action:'แก้ HTTP error หรือ URL ใน sitemap แล้วสแกนใหม่',evidence:'HTTP '+status}]};
  }
...
function duplicateIssues(pages) {
  for(const field of ['title','description']) {
    const groups=new Map();for(const p of pages){if(!scoredPage(p)||!p[field])continue;const key=p[field].toLocaleLowerCase().trim();groups.set(key,[...(groups.get(key)||[]),p]);}
...
function createIntelligence({dataDir,base,fetcher=fetchPage,cooldownMs=60000,persistent}) {
  ...
  const previousFor=(history,url)=>history.find(s=>s.site===base&&s.scoringVersion===VERSION&&s.pages.some(p=>p.url===url&&scoredPage(p)));
  async function scan() {

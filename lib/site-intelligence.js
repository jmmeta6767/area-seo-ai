...
function createIntelligence({dataDir,base,fetcher=fetchPage,cooldownMs=60000,persistent}) {
  ...
  async function scan() {
    ...
    try {
      ...
      const rootMap=new URL('/sitemap.xml',base).href,queue=[rootMap],queued=new Set(queue);
      let sitemapsRead=0;
      while(queue.length){

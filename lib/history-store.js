...
function createHistoryStore({dataDir,persistent}){
 ...
 async function replace(data,{skipPersistent=false}={}){validateHistory(data);if(configured&&!skipPersistent){if(!ready)throw Object.assign(Error('Website history storage is not ready'),{status:503});await persistent.set('site_history',data);}if(configured){memory=clone(data);ready=true;writeError=false;cache(data);}else writeLocal(data);return clone(data);}
 ...
 return {init,read,save,replace,status};
}

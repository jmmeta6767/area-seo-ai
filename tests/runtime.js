...
async function main() {
  ...
  assert.equal((await request('/api/health')).data.version, '3.6.0');
  ...
  const readiness=(await request('/api/admin/readiness')).data; assert.equal(readiness.ready,false); assert(readiness.blockers.includes('persistent_storage')); assert(readiness.blockers.includes('google_performance'));
  ...
  const context = vm.createContext({document:{getElementById:el}, window:{}, fetch:async(url, opts)=>{
    let data={};
    if(url==='/api/health') data={version:'3.6.0'};

...
async function main(){
 ...
 const restarted=createQualityHistory({dataDir:path.join(root,'empty'),persistent});await restarted.init();assert.equal(restarted.list().length,11);const original11=clone(saved);
 const replacement={version:1,items:[clone(first)]};await restarted.replace(replacement);assert.equal(restarted.list().length,1);assert.equal(saved.items.length,1);await restarted.replace({version:1,items:[clone(first),clone(first)].map((x,i)=>({...x,id:'replace-'+i}))});assert.equal(restarted.list().length,2);await restarted.replace(original11);assert.equal(saved.items.length,11);
 fail=true;
